...
const ProductShowcase = () => {
    ...
    useEffect(() => {
        const ctx = gsap.context(() => {
            ...
            if (isMobile) {
                // Mobile: Horizontal scroll with pinning
                    ...
                    <div key={index} className={`h-screen md:h-auto md:min-h-screen w-[100vw] flex-shrink-0 relative flex items-center justify-center item-trigger-${index} overflow-hidden pt-28 md:pt-0`}>
                        <div className="container mx-auto px-10 sm:px-12 md:px-24 grid grid-cols-1 md:grid-cols-2 gap-16 md:gap-20 items-center w-full py-12 md:py-0">
                            ...
                            <div className="relative order-1 md:order-2 w-full flex justify-center pb-8 md:pb-0">
                                <div className="aspect-square sm:aspect-[4/5] w-full max-w-[280px] sm:max-w-[400px] bg-white/5 overflow-hidden rounded-sm border border-white/10 group shadow-[0_0_100px_rgba(212,175,55,0.05)] cursor-pointer transition-transform" onTouchStart={(e) => e.currentTarget.classList.add('touched')} onTouchEnd={(e) => e.currentTarget.classList.remove('touched')}>
                                    <img
                                        src={product.image}
                                        className="w-full h-full object-cover transition-all duration-1000 ease-out group-hover:scale-110 group-hover:rotate-1 group-touched:scale-110 group-touched:rotate-1"
                                        alt={product.title}
                                    />
                                    <div className="absolute inset-0 bg-gradient-to-t from-black/60 via-transparent to-transparent opacity-0 group-hover:opacity-100 group-touched:opacity-100 transition-opacity duration-700" />
                                </div>
                            </div>
                        </div>
                    </div>
