...
const ProductShowcase = () => {
    ...
    useEffect(() => {
        const ctx = gsap.context(() => {
            ...
            // Simple Fade-In for texts on view
    ...
    return (
        <section id="showcase" ref={sectionRef} className="relative overflow-hidden bg-black border-y border-white/5 z-10">
            <div ref={containerRef} className="flex h-screen w-max items-center">
                {products.map((product, index) => (
                    <div key={index} className={`h-screen w-screen flex-shrink-0 relative flex items-center justify-center item-trigger-${index}`}>
                        <div className="container mx-auto px-12 md:px-24 grid grid-cols-1 md:grid-cols-2 gap-12 items-center">

                            <div className={`item-text-${index} space-y-8 max-w-xl`}>
                                <div className="flex items-center gap-4 text-gold-accent/60">
                                    <span className="text-[10px] uppercase tracking-[0.4em] font-display font-medium">{product.tag}</span>
                                    <div className="h-[1px] w-12 bg-gold-accent/30" />
                                    <span className="text-[10px] uppercase tracking-[0.4em] font-display">{product.year}</span>
                                </div>

                                <h2 className="text-5xl md:text-8xl font-bold text-white tracking-tighter leading-[0.9] font-display">
                                    {product.title}
                                </h2>
                                ...
                                <div className="pt-8">
                                    <a href="#menu" className="inline-flex items-center gap-4 text-xs uppercase tracking-[0.3em] text-white hover:text-gold-accent transition-colors group">
                                        <span>Explore the Recipe</span>
                                        <svg width="24" height="24" viewBox="0 0 24 24" fill="none" className="group-hover:translate-x-2 transition-transform duration-500">
                            ...
                            <div className="relative">
                                <div className="aspect-[4/5] w-full bg-white/5 overflow-hidden rounded-sm border border-white/10 group shadow-[0_0_100px_rgba(212,175,55,0.05)]">
                                    <img
                                        src={product.image}
